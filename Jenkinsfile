...
pipeline {
	...
	environment {
		...
		GITHUB_URL ='https://github.com/microchip-pic-avr-examples/pic16f15276-cnano-uart-io-expander-client-mplab-mcc'
		BITBUCKET_URL = 'https://bitbucket.microchip.com/scm/ebe/pic16f15276-cnano-uart-io-expander-client-mplab-mcc.git'
		...
		stage('setup') {
		    steps {
				script {
					...
					execute("python ./tool-github-deploy/tool-github-deploy/tool-github-deploy.py -rlo=true -gpat=${USER_PASS}  -rpn=${githubObj.repoName} -rltv=${env.TAG_NAME} -rltt=${env.TAG_NAME} -rlua=./${githubObj.repoName}/${githubObj.repoName}.X/dist/default/production/${githubObj.repoName}.X.production.hex")
				}
            }
		}
	}
